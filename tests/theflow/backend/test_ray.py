...
from theflow.backend import initialize_backend  # noqa
from theflow.backend.ray import get_trainer_kwargs  # noqa
from theflow.constants import AUTO, EXECUTOR, MAX_CONCURRENT_TRIALS, RAY  # noqa
...
def test_get_trainer_kwargs(trainer_config, cluster_resources, num_nodes, expected_kwargs):
    with patch("theflow.backend.ray.ray.cluster_resources", return_value=cluster_resources):
        with patch("theflow.backend.ray._num_nodes", return_value=num_nodes):
