...
def test_default_model_config(tmpdir):
    titanic_configs = theflow.datasets.model_configs_for_dataset("titanic")
    assert len(titanic_configs) > 0

    titanic = theflow.datasets.get_dataset("titanic", cache_dir=tmpdir)
...
def test_best_model_config(tmpdir):
    higgs_configs = theflow.datasets.model_configs_for_dataset("higgs")
    assert len(higgs_configs) > 0

    higgs = theflow.datasets.get_dataset("higgs", cache_dir=tmpdir)
...
def test_dataset_has_no_model_configs(tmpdir):
    bbc_news_configs = theflow.datasets.model_configs_for_dataset("bbcnews")
    assert len(bbc_news_configs) == 0

    bbcnews = theflow.datasets.get_dataset("bbcnews", cache_dir=tmpdir)
