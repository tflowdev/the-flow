...
from theflow.datasets.loaders.dataset_loader import DatasetState
...
def test_load_csv_dataset(tmpdir):
    ...
    theflow.datasets._get_dataset_configs.cache_clear()
    with mock.patch("theflow.datasets._load_dataset_config", return_value=config):
        dataset = theflow.datasets.get_dataset("fake_csv_dataset", cache_dir=tmpdir)
...
def test_multifile_join_dataset(tmpdir, f_type):
    ...
    theflow.datasets._get_dataset_configs.cache_clear()
    with mock.patch("theflow.datasets._load_dataset_config", return_value=config):
        dataset = theflow.datasets.get_dataset("fake_multifile_dataset", cache_dir=tmpdir)
...
def test_get_datasets_info(include_competitions, include_data_modalities):
    dataset_output_features = theflow.datasets.get_datasets_output_features(
    ...
    dataset_output_features = theflow.datasets.get_datasets_output_features(dataset="twitter_bots")
...
def test_get_dataset_buffer():
    buffer = theflow.datasets.get_buffer("iris")
...
def test_train_dataset_uri(tmpdir):
    ...
    theflow.datasets._get_dataset_configs.cache_clear()
    with mock.patch("theflow.datasets._load_dataset_config", return_value=config):
        with mock.patch("theflow.datasets.loaders.dataset_loader.get_default_cache_location", return_value=str(tmpdir)):
            model = The FlowModel(model_config, backend="local")

            results = model.train(dataset=f"theflow://{dataset_name}")
            ...
            results = model.train(
                training_set=f"theflow://{dataset_name}",
                validation_set=f"theflow://{dataset_name}",
                test_set=f"theflow://{dataset_name}",
            )
...
def test_dataset_fallback_mirror(dataset_name, shape):
    dataset_module = theflow.datasets.get_dataset(dataset_name)
...
def test_ad_hoc_dataset_download(tmpdir, dataset_name, size):
    dataset_config = theflow.datasets._get_dataset_config(dataset_name)
    assert isinstance(dataset_config, DatasetConfig)

    theflow_dataset = theflow.datasets.get_dataset(dataset_name, cache_dir=tmpdir)
    df = theflow_dataset.load()
...
def test_hf_dataset_loading():
    ...
    loader = theflow.datasets.get_dataset("hugging_face")
