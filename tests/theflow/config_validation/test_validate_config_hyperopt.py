...
import theflow.schema.hyperopt.parameter  # noqa: F401
import theflow.schema.hyperopt.scheduler  # noqa: F401
import theflow.schema.hyperopt.search_algorithm  # noqa: F401
...
from theflow.schema.hyperopt import utils
...
@pytest.mark.parametrize(
    ...
    [
        ([], False),
        ([("theflow", "theflow")], False),
        ([("theflow", "theflow"), ("marshmallow", "marshmallow")], False),
        ([("fake_dependency", "fake_dependency")], True),
        ([("theflow", "theflow"), ("fake_dependency", "fake_dependency")], True),
    ],
)
def test_check_scheduler_dependencies_installed(dependencies, raises_exception):
    ...
    with patch("theflow.schema.hyperopt.utils.get_scheduler_dependencies", return_value=dependencies):
        if raises_exception:
...
def test_check_search_algorithm_dependencies_installed(dependencies, raises_exception):
    ...
    with patch("theflow.schema.hyperopt.utils.get_search_algorithm_dependencies", return_value=dependencies):
        if raises_exception:
            with pytest.raises(ConfigValidationError):
                ModelConfig.from_dict(config)
        else:
