...
import theflow.schema.encoders.utils as schema_encoders_utils
from theflow.api import The FlowModel
from theflow.constants import ENCODER, ENCODER_OUTPUT, MODEL_ECD, NAME, TEXT, TRAINER
from theflow.encoders import text_encoders
...
def get_mismatched_config_params(theflow_results_dir, theflow_model):
    saved_config_dict = load_json(os.path.join(theflow_results_dir, MODEL_FILE_NAME, MODEL_HYPERPARAMETERS_FILE_NAME))
    ...
    for input_feature_config in saved_config_obj.input_features.to_list():
        ...
        encoder_config_from_model = theflow_model.model.input_features.get(feature_name).encoder_obj.config.to_dict()
...
def test_hf_theflow_model_e2e(tmpdir, csv_filename, encoder_name):
    """Tests HuggingFace encoders end-to-end.

    This test validates the following:
        1. Encoder config defaults are compatible with The Flow experiments.
        2. The Flow correctly updates the encoder config with the parameters introduced by the HF encoder.
        3. The Flow correctly loads checkpoints containing HF encoder weights.
    """
    input_features = [
        text_feature(
            encoder={
    ...
    model = The FlowModel(config=config, backend=LocalTestBackend())

    with mock.patch(
        "theflow.encoders.text_encoders.load_pretrained_hf_model_with_hub_fallback",
        side_effect=_load_pretrained_hf_model_no_weights,
    ):
        # Validates that the defaults associated with the encoder are compatible with The Flow training.
        ...
        The FlowModel.load(os.path.join(results_dir, MODEL_FILE_NAME))
    clear_huggingface_cache()
...
@pytest.mark.parametrize("encoder_name", HF_ENCODERS)
def test_hf_theflow_model_reduce_options(tmpdir, csv_filename, encoder_name, reduce_output):
    input_features = [
        text_feature(
            preprocessing={
...
def test_hf_theflow_model_auto_transformers(tmpdir, csv_filename, pretrained_model_name_or_path):
