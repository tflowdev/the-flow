...
def test_dataset_synthesizer_output_feature_decoder():
    ...
    The FlowModel(config=config, logging_level=logging.INFO)


@pytest.mark.parametrize(
    "dataset_1k_url",
    [
        private_param(["s3://theflow-tests/datasets/synthetic_1k.csv"]),
        private_param(["s3://theflow-tests/datasets/synthetic_1k.parquet"]),
    ],
)
