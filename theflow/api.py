...
from theflow.backend import Backend, initialize_backend, provision_preprocessing_workers
...
from theflow.data.cache.types import CacheableDataset
from theflow.data.dataset.base import Dataset
from theflow.data.postprocessing import convert_predictions, postprocess
from theflow.data.preprocessing import load_metadata, preprocess_for_prediction, preprocess_for_training
from theflow.datasets import load_dataset_uris
from theflow.features.feature_registries import update_config_with_metadata, update_config_with_model
from theflow.globals import (
    LUDWIG_VERSION,
    MODEL_FILE_NAME,
...
from theflow.models.calibrator import Calibrator
from theflow.models.inference import InferenceModule, save_theflow_model_for_inference
from theflow.models.predictor import (
    calculate_overall_stats,
    print_evaluation_stats,
    save_evaluation_stats,
    save_prediction_outputs,
)
from theflow.models.registry import model_type_registry
from theflow.schema.model_config import ModelConfig
from theflow.types import ModelConfigDict, TrainingSetMetadataDict
from theflow.upload import get_upload_registry
from theflow.utils import metric_utils
from theflow.utils.backward_compatibility import upgrade_config_dict_to_latest_version
from theflow.utils.config_utils import get_preprocessing_params
from theflow.utils.data_utils import (
    figure_data_format,
...
from theflow.utils.dataset_utils import generate_dataset_statistics
from theflow.utils.defaults import default_random_seed
from theflow.utils.fs_utils import makedirs, path_exists, upload_output_directory
from theflow.utils.heuristics import get_auto_learning_rate
from theflow.utils.llm_utils import create_text_streamer, TextStreamer
from theflow.utils.misc_utils import (
    get_commit_hash,
    get_file_names,
    get_from_registry,
...
from theflow.utils.print_utils import print_boxed
from theflow.utils.tokenizers import HFTokenizer
from theflow.utils.torch_utils import DEVICE
from theflow.utils.trainer_utils import get_training_report
from theflow.utils.types import DataFrame, TorchDevice
from theflow.utils.upload_utils import HuggingFaceHub
...
@PublicAPI
class The FlowModel:
    """Class that allows access to high level The Flow functionalities.
    ...
    train_stats, _, _ = theflow_model.train(dataset=file_path)
    ```

    or

    ```python
    train_stats, _, _ = theflow_model.train(dataset=dataframe)
    ```
    ...
    ```python
    predictions, _ = theflow_model.predict(dataset=file_path)
    ```

    or

    ```python
    predictions, _ = theflow_model.predict(dataset=dataframe)
    ```

    Evaluation:

    ```python
    eval_stats, _, _ = theflow_model.evaluate(dataset=file_path)
    ```

    or

    ```python
    eval_stats, _, _ = theflow_model.evaluate(dataset=dataframe)
    ```
    """
    ...
    def __init__(
        ...
        """Constructor for the The Flow Model class.
        ...
        :param callbacks: (list, default: `None`) a list of
              `theflow.callbacks.Callback` objects that provide hooks into the
               The Flow pipeline.
        ...
        if (
            ...
            # Category output features require a vocabulary. The LLM The FlowModel should be initialized with
            # model.train(dataset).
    ...
    def _initialize_llm(self, random_seed: int = default_random_seed):
        ...
        """
        self.model = The FlowModel.create_model(self.config_obj, random_seed=random_seed)
    ...
    def train(
        ...
        **kwargs,
        ...
        :param skip_save_model: (bool, default: `False`) disables
            ...
            improves. By default The Flow saves model weights after each epoch
        ...
        :param skip_save_progress: (bool, default: `False`) disables saving
            progress each epoch. By default The Flow saves weights and stats
            after each epoch for enabling resuming of training, but if
            the model is really big that can be time consuming and will uses
            twice as much space, use this parameter to skip it, but training
            cannot be resumed later on.
        :param skip_save_log: (bool, default: `False`) disables saving
            TensorBoard logs. By default The Flow saves logs for the TensorBoard,
    ...
    def save_dequantized_base_model(self, save_path: str) -> None:
        ...
        if not self.config_obj.quantization:
            raise ValueError(
                "Quantization is not enabled in your The Flow model config. "
        ...
        if not self.model:
            self.model = The FlowModel.create_model(self.config_obj)
    ...
    def generate(
        ...
        if not torch.cuda.is_available():
            ...
            # https://github.com/theflow-ai/theflow/issues/3695.
    ...
    def predict(
        ...
        **kwargs,
        ...
        :param generation_config: (Dict, default: `None`) config for the generation of the
            ...
            ignored. See
            [Large Language Models](https://theflow.ai/latest/configuration/large_language_model/#generation) under
    ...
    def load(
    ...
    ) -> "The FlowModel":  # return is an instance of theflow.api.The FlowModel class
        ...
        :return: (The FlowModel) a The FlowModel object
        ...
        theflow_model.model = The FlowModel.create_model(config_obj)

        # load model weights
        theflow_model.load_weights(model_dir, from_checkpoint)

        # The LoRA layers appear to be loaded again (perhaps due to a potential bug); hence, we merge and unload again.
        if theflow_model.is_merge_and_unload_set():
            # For an LLM model trained with a LoRA adapter, handle merge and unload postprocessing directives.
            theflow_model.model.merge_and_unload(progressbar=config_obj.adapter.postprocessor.progressbar)

        # load train set metadata
        theflow_model.training_set_metadata = backend.broadcast_return(
            lambda: load_metadata(os.path.join(model_dir, TRAIN_SET_METADATA_FILE_NAME))
        )

        return theflow_model
    ...
    def load_weights(
        ...
        ```python
        theflow_model.load_weights(model_dir)
        ```
        """
        if self.backend.is_coordinator():
            if from_checkpoint:
    ...
    def upload_to_hf_hub(
        ...
        repo_type: str = "model",
        private: bool = False,
        commit_message: str = "Upload trained [The Flow](https://theflow.ai/latest/) model weights",
    ...
    def to_torchscript(
        ...
        if model_only:
            ...
            inference_module = InferenceModule.from_theflow_model(
    ...
    def save_torchscript(
        ...
        save_theflow_model_for_inference(
            save_path,
            self.model,
            self.config_obj.to_dict(),
            self.training_set_metadata,
    ...
    def create_model(config_obj: Union[ModelConfig, dict], random_seed: int = default_random_seed) -> BaseModel:
        ...
        # Inputs
        :param config_obj: (Union[Config, dict]) The Flow config object
        :param random_seed: (int, default: theflow default random seed) Random
            seed used for weights initialization,
            splits and any other random function.

        # Return
        :return: (theflow.models.BaseModel) Instance of the The Flow model object.
        """
    ...
    def set_logging_level(logging_level: int) -> None:
        ...
        :return: `None`
        """
        logging.getLogger("theflow").setLevel(logging_level)
...
def kfold_cross_validate(
    ...
    **kwargs,
    ...
    :param config: (Union[dict, str]) model specification
           ...
           [User Guide](http://theflow.ai/user_guide/#model-config)
           for details.
    :param dataset: (Union[str, dict, pandas.DataFrame], default: `None`)
    ...
    :param skip_save_log: (bool, default: `False`) disables saving TensorBoard
           logs. By default The Flow saves logs for the TensorBoard, but if it
    ...
    # use The Flow's utility to facilitate creating a dataframe
...
def get_experiment_description(
    ...
    description["theflow_version"] = LUDWIG_VERSION
