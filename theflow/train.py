...
from theflow.constants import CONTINUE_PROMPT, HYPEROPT, HYPEROPT_WARNING
from theflow.contrib import add_contrib_callback_args
from theflow.globals import LUDWIG_VERSION
from theflow.utils.data_utils import load_config_from_str, load_yaml
...
def train_cli(
    ...
    **kwargs
    ...
    """*train* defines the entire training procedure used by The Flow's
    internals. Requires most of the parameters that are taken into the model.
    Builds a full theflow model and performs the training.
    ...
    if HYPEROPT in config:
        ...
        # TODO: https://github.com/theflow-ai/theflow/issues/2633
...
def cli(sys_argv):
    parser = argparse.ArgumentParser(
        description="This script trains a model", prog="theflow train", usage="%(prog)s [options]"
    )
    ...
    parser.add_argument(
        ...
        "By default The Flow saves  weights after each epoch "
        "the validation metric (improves, but  if the model is really big "
        "that can be time consuming. If you do not want to keep "
        "the weights and just find out what performance a model can get "
        "with a set of hyperparameters, use this parameter to skip it",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="disables saving weights after each epoch. By default theflow saves "
