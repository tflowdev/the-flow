...
from theflow.explain.explanation import ExplanationsResult
from theflow.explain.util import get_pred_col, replace_layer_with_copy
...
class WrapperModule(torch.nn.Module):
    ...
    Unlike The Flow's ECD class, this wrapper takes individual args as inputs to the forward function. We derive the order
    ...
    def __init__(self, model: ECD, target: str):
        ...
        self.model = model
        self.target = target
        self.input_maps = The FlowFeatureDict()
    ...
    def forward(self, *args):
        ...
        input_features: The FlowFeatureDict = self.model.input_features
        inputs = {
...
class IntegratedGradientsExplainer(Explainer):
    def explain(self) -> ExplanationsResult:
        ...
        input_features: The FlowFeatureDict = self.model.model.input_features
...
def get_input_tensors(
    model: The FlowModel, input_set: pd.DataFrame, run_config: ExplanationRunConfig
) -> List[torch.Tensor]:
    """Convert the input data into a list of variables, one for each input feature.

    # Inputs

    :param model: The The FlowModel to use for encoding.
...
def get_baseline(model: The FlowModel, sample_encoded: List[Variable]) -> List[torch.Tensor]:
    # TODO(travis): pre-compute this during training from the full training dataset.
    input_features: The FlowFeatureDict = model.model.input_features
...
def get_total_attribution(
    ...
    Args:
        model: The The Flow model to explain.
...
def get_token_attributions(
    ...
    Args:
        model: The The FlowModel used to generate the attributions.
