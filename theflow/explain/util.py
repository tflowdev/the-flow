...
from theflow.constants import COLUMN, INPUT_FEATURES, PREPROCESSING, SPLIT
...
def prepare_data(model: The FlowModel, inputs_df: pd.DataFrame, sample_df: pd.DataFrame, target: str):
    config = model.config
    feature_cols = [feature[COLUMN] for feature in config[INPUT_FEATURES]]
    if SPLIT in config.get(PREPROCESSING, {}):
        # Keep columns required for The Flow preprocessing
...
def get_feature_name(model: The FlowModel, target: str) -> str:
    t = target.lower()
