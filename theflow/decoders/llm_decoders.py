...
from theflow.constants import CATEGORY, LOGITS, PREDICTIONS, PROBABILITIES, TEXT
from theflow.decoders.base import Decoder
from theflow.decoders.registry import register_decoder
from theflow.decoders.utils import extract_generated_tokens
from theflow.schema.decoders.llm_decoders import CategoryExtractorDecoderConfig, TextExtractorDecoderConfig
from theflow.utils.strings_utils import get_tokenizer
    ...
    def __call__(self, decoded_input: str) -> Union[str, None]:
        ...
        for label, label_def in self.match.items():
            ...
            if label_def_type == "contains":
                ...
                raise ValueError(
                    f"{label_def_type} is not a valid match `type`. The Flow "
