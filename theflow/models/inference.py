...
from theflow.constants import NAME, POSTPROCESSOR, PREDICTOR, PREPROCESSOR, TYPE
from theflow.data.postprocessing import convert_dict_to_df
from theflow.data.preprocessing import load_metadata
from theflow.features.feature_registries import get_input_type_registry
from theflow.features.feature_utils import get_module_dict_key_from_name, get_name_from_module_dict_key
from theflow.globals import MODEL_HYPERPARAMETERS_FILE_NAME, TRAIN_SET_METADATA_FILE_NAME
from theflow.types import ModelConfigDict, TrainingSetMetadataDict
from theflow.utils import output_feature_utils
from theflow.utils.data_utils import load_json, save_json
from theflow.utils.inference_utils import get_filename_from_stage, to_inference_module_input_from_dataframe
from theflow.utils.misc_utils import get_from_registry
from theflow.utils.output_feature_utils import get_feature_name_from_concat_name, get_tensor_name_from_concat_name
from theflow.utils.torch_utils import DEVICE
from theflow.utils.types import TorchDevice, TorchscriptPreprocessingInput
...
class InferenceModule(nn.Module):
    ...
    def predict(
        ...
        """Predict on a batch of data with an interface similar to The FlowModel.predict."""
        ...
        return preds, None  # Second return value is for compatibility with The FlowModel.predict

    @torch.jit.unused
    @classmethod
    def from_theflow_model(
        cls: "InferenceModule",
        model: "BaseModel",
        ...
        """Create an InferenceModule from a trained The FlowModel."""
        if device is None:
            logger.info(f'No device specified. Loading using device "{DEVICE}".')
            device = DEVICE

        stage_to_module = _init_inference_stages_from_theflow_model(
...
class _InferencePreprocessor(nn.Module):
    ...
    TODO(geoffrey): Implement torchscript-compatible feature_utils.The FlowFeatureDict to replace
...
def save_theflow_model_for_inference(
    save_path: str,
    model: "BaseModel",
    config: ModelConfigDict,
    training_set_metadata: TrainingSetMetadataDict,
    ...
    """Saves a The FlowModel (a BaseModel model, config, and training_set_metadata) for inference."""
    if device is None:
...
def _init_inference_stages_from_theflow_model(
    model: "BaseModel",
    config: ModelConfigDict,
    training_set_metadata: TrainingSetMetadataDict,
    device: TorchDevice,
    ...
    """Initializes inference stage modules from a The FlowModel (a BaseModel model, config, and
