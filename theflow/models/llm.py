...
from theflow.accounting.used_tokens import get_used_tokens_for_llm
from theflow.constants import IGNORE_INDEX_TOKEN_ID, LOGITS, MODEL_LLM, PREDICTIONS, TEXT, USED_TOKENS
from theflow.features.base_feature import ModuleWrapper, OutputFeature
from theflow.features.feature_utils import The FlowFeatureDict
from theflow.features.text_feature import TextOutputFeature
from theflow.globals import MODEL_WEIGHTS_FILE_NAME
from theflow.models.base import BaseModel
from theflow.modules.training_hooks import NEFTuneHook
from theflow.schema.features.base import BaseOutputFeatureConfig, FeatureCollection
from theflow.schema.model_types.llm import LLMModelConfig
from theflow.utils.augmentation_utils import AugmentationPipelines
from theflow.utils.data_utils import clear_data_cache
from theflow.utils.llm_quantization_utils import convert_quantized_linear_to_linear
from theflow.utils.llm_utils import (
    add_left_padding,
    generate_merged_ids,
    get_context_len,
...
from theflow.utils.output_feature_utils import set_output_feature_tensor
from theflow.utils.tokenizers import HFTokenizer
from theflow.utils.torch_utils import reg_loss

logger = logging.getLogger(__name__)


class DictWrapper:
    """Wrapper for a The FlowFeatureDict module that allows for iteration over keys.
    ...
    def __init__(self, obj: The FlowFeatureDict):
        self.obj = obj
...
class LLM(BaseModel):
    ...
    def create_feature_dict(self) -> DictWrapper:
        return DictWrapper(The FlowFeatureDict())
    ...
    def save_base_model(self, save_path):
        ...
        if self.config_obj.trainer.type != "none":
            ...
            # need to be saved if inference is to be done using The FlowModel.predict(), the rationale for saving the
            # tokenizer to HuggingFace Hub is to provide access to models fine-tuned and persisted to HuggingFace Hub
            # using The Flow at a later time, with the ability to perform inference, independently of The Flow itself.
    ...
    def save_dequantized_base_model(self, save_path: str) -> None:
        ...
        if isinstance(self.model, PeftModel):
            ...
            logger.warning(
                ...
                "Reload the model via The FlowModel.load() to use your trained adapter layers for inference."
            )
