...
from theflow.combiners.combiners import Combiner
from theflow.constants import COMBINED, LOSS, NAME
from theflow.encoders.base import Encoder
from theflow.features.base_feature import create_passthrough_input_feature, InputFeature, ModuleWrapper, OutputFeature
from theflow.features.feature_registries import get_input_type_registry, get_output_type_registry
from theflow.features.feature_utils import The FlowFeatureDict
from theflow.modules.metric_modules import The FlowMetric
from theflow.modules.training_hooks import TrainingHook
from theflow.schema.features.base import BaseInputFeatureConfig, BaseOutputFeatureConfig, FeatureCollection
from theflow.utils.algorithms_utils import topological_sort_feature_dependencies
from theflow.utils.metric_utils import get_scalar_from_theflow_metric
from theflow.utils.misc_utils import get_from_registry
from theflow.utils.torch_utils import DEVICE, The FlowModule, reg_loss
from theflow.utils.types import TorchDevice

logger = logging.getLogger(__name__)


class BaseModel(The FlowModule, metaclass=ABCMeta):
    """Base model for use in The FlowModule.
    ...
    def create_feature_dict(self) -> The FlowFeatureDict:
        """Creates and returns a The FlowFeatureDict."""
        return The FlowFeatureDict()
    ...
    @property
    def eval_loss_metric(self) -> The FlowMetric:
        return self._eval_loss_metric.module

    @eval_loss_metric.setter
    def eval_loss_metric(self, value: The FlowMetric) -> None:
        self._eval_loss_metric.module = value

    @property
    def eval_additional_losses_metrics(self) -> The FlowMetric:
    ...
    def get_metrics(self) -> Dict[str, Dict[str, float]]:
        ...
        all_of_metrics[COMBINED] = {
            LOSS: get_scalar_from_theflow_metric(self.eval_loss_metric)
            + get_scalar_from_theflow_metric(self.eval_additional_losses_metrics)
        }
