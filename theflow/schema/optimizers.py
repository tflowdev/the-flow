...
import theflow.schema.utils as schema_utils
from theflow.api_annotations import DeveloperAPI
from theflow.schema.metadata import OPTIMIZER_METADATA
from theflow.schema.metadata.parameter_metadata import convert_metadata_to_json, ParameterMetadata
from theflow.schema.utils import theflow_dataclass
from theflow.utils.registry import Registry
...
class BaseOptimizerConfig(schema_utils.BaseMarshmallowConfig, ABC):
    ...
    type: str
    """Name corresponding to an optimizer `theflow.modules.optimization_modules.optimizer_registry`.
...
class SGDOptimizerConfig(BaseOptimizerConfig):
    ...
    """Must be 'sgd' - corresponds to name in `theflow.modules.optimization_modules.optimizer_registry` (default:
       'sgd')"""
...
class LBFGSOptimizerConfig(BaseOptimizerConfig):
    ...
    """Must be 'lbfgs' - corresponds to name in `theflow.modules.optimization_modules.optimizer_registry` (default:
       'lbfgs')"""
...
class AdamOptimizerConfig(BaseOptimizerConfig):
    ...
    """Must be 'adam' - corresponds to name in `theflow.modules.optimization_modules.optimizer_registry`
       (default: 'adam')"""
...
class AdamWOptimizerConfig(BaseOptimizerConfig):
    ...
    """Must be 'adamw' - corresponds to name in `theflow.modules.optimization_modules.optimizer_registry`
       (default: 'adamw')"""
...
class AdadeltaOptimizerConfig(BaseOptimizerConfig):
    ...
    """Must be 'adadelta' - corresponds to name in `theflow.modules.optimization_modules.optimizer_registry`
       (default: 'adadelta')"""
...
class AdagradOptimizerConfig(BaseOptimizerConfig):
    ...
    """Must be 'adagrad' - corresponds to name in `theflow.modules.optimization_modules.optimizer_registry`
       (default: 'adagrad')"""
...
class AdamaxOptimizerConfig(BaseOptimizerConfig):
    ...
    """Must be 'adamax' - corresponds to name in `theflow.modules.optimization_modules.optimizer_registry`
       (default: 'adamax')"""
...
class RMSPropOptimizerConfig(BaseOptimizerConfig):
    ...
    """Must be 'rmsprop' - corresponds to name in `theflow.modules.optimization_modules.optimizer_registry`
       (default: 'rmsprop')"""
...
def get_optimizer_conds():
    ...
    `theflow.modules.optimization_modules.optimizer_registry`."""
    conds = []
...
def OptimizerDataclassField(default="adam", description="", parameter_metadata: ParameterMetadata = None):
    ...
    class OptimizerSelection(schema_utils.TypeSelection):
        ...
        `theflow.modules.optimization_modules.optimizer_registry` and creates a corresponding `oneOf` JSON schema
        for external usage."""
...
def GradientClippingDataclassField(description: str, default: Dict = {}):
    """Returns custom dataclass field for `theflow.modules.optimization_modules.GradientClippingConfig`. Allows
    `None` by default.
    ...
    class GradientClippingMarshmallowField(fields.Field):
        ...
        Deserializes a dict to a valid instance of `theflow.modules.optimization_modules.GradientClippingConfig` and
