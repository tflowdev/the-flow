...
from theflow.schema.hyperopt import utils as hyperopt_utils
...
class BaseSchedulerConfig(schema_utils.BaseMarshmallowConfig, ABC):
    ...
    type: str
    """Name corresponding to a scheduler in `theflow.schema.hyperopt.scheduler.scheduler_registry`.
...
def get_scheduler_conds():
    ...
    `theflow.schema.hyperopt.scheduler_registry`."""
    conds = []
...
def SchedulerDataclassField(default={"type": "fifo"}, description="Hyperopt scheduler settings."):
    ...
    `theflow.schema.hyperopt.scheduler.scheduler_registry`. Sets default scheduler to 'fifo'.
    ...
    class SchedulerMarshmallowField(fields.Field):
        ...
        `theflow.schema.hyperopt.scheduler_registry` and creates a corresponding `oneOf` JSON schema for external
        usage."""
