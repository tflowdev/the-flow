...
class BaseSearchAlgorithmConfig(schema_utils.BaseMarshmallowConfig):
    ...
    def set_random_state(self, theflow_random_state: int) -> None:
        ...
        with the The Flow random state. This method uses a registry of random state field names to provide a single
        ...
        if rs_field is not None and self.__getattribute__(rs_field) is None:
            self.__setattr__(rs_field, theflow_random_state)
