...
# The smallest batch size that is supported on The Flow.
...
HYPEROPT_WARNING = (
    "You are running the theflow train command but there’s a hyperopt section present in your config. "
    "It will be ignored. If you want to run hyperopt you should use the following command: theflow "
    "hyperopt\n\n"
)
...
LUDWIG_VERSION = "theflow_version"
...
# If `use_torch_profiler=True` in The FlowProfiler, LUDWIG_TAG is prepended to the specified experiment tag
# (The FlowProfiler(tag="...", ..)). This edited tag is passed in to `torch.profiler.record_function` so we can
