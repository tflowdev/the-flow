...
from theflow.constants import AUDIO, COLUMN
...
from theflow.utils.server_utils import NumpyJSONResponse
...
def server(model, allowed_origins=None):
    ...
    @app.get("/")
    def check_health():
        return NumpyJSONResponse({"message": "The Flow server is up"})
...
def run_server(
    ...
    model = The FlowModel.load(model_path, backend="local")
...
def cli(sys_argv):
    parser = argparse.ArgumentParser(
        description="This script serves a pretrained model", prog="theflow serve", usage="%(prog)s [options]"
    )
    ...
    global logger
    logger = logging.getLogger("theflow.serve")

    print_theflow("Serve", LUDWIG_VERSION)
