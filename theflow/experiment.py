...
from theflow.api import kfold_cross_validate, The FlowModel
from theflow.backend import ALL_BACKENDS, Backend, initialize_backend
from theflow.callbacks import Callback
from theflow.constants import CONTINUE_PROMPT, FULL, HYPEROPT, HYPEROPT_WARNING, TEST, TRAINING, VALIDATION
from theflow.contrib import add_contrib_callback_args
from theflow.globals import LUDWIG_VERSION
from theflow.utils.data_utils import load_config_from_str, load_yaml, save_json
from theflow.utils.defaults import default_random_seed
from theflow.utils.print_utils import get_logging_level_registry, print_theflow, query_yes_no
...
def kfold_cross_validate_cli(
    ...
    **kwargs,
    ...
    :param config: (Union[str, dict], default: None) a dictionary or file path
           ...
           (http://theflow.ai/user_guide/#model-config) for details.
...
def cli(sys_argv):
    parser = argparse.ArgumentParser(
        description="This script trains and evaluates a model", prog="theflow experiment", usage="%(prog)s [options]"
    )
    ...
    parser.add_argument(
        ...
        "the model improves. "
        "By default The Flow saves model weights after each epoch "
        "the validation metric improves, but if the model is really big "
        "that can be time consuming. If you do not want to keep "
        "the weights and just find out what performance a model can get "
        "with a set of hyperparameters, use this parameter to skip it,"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="disables saving progress each epoch. By default The Flow saves "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="disables saving TensorBoard logs. By default The Flow saves "
