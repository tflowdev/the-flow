...
from theflow.distributed.base import DistributedStrategy, LocalStrategy
...
from theflow.models.predictor import Predictor
from theflow.modules.lr_scheduler import LRScheduler
from theflow.modules.metric_modules import get_improved_fn, get_initial_validation_value
from theflow.modules.metric_registry import get_metric_objective
from theflow.modules.optimization_modules import create_clipper
from theflow.progress_bar import The FlowProgressBar
from theflow.schema.trainer import ECDTrainerConfig
from theflow.trainers.base import BaseTrainer
from theflow.trainers.registry import register_trainer
from theflow.types import ModelConfigDict
from theflow.utils import time_utils
from theflow.utils.batch_size_tuner import BatchSizeEvaluator
from theflow.utils.checkpoint_utils import Checkpoint, CheckpointManager
from theflow.utils.config_utils import get_quantization
from theflow.utils.data_utils import load_json
from theflow.utils.defaults import default_random_seed
from theflow.utils.fs_utils import path_exists
from theflow.utils.llm_utils import update_embedding_layer
from theflow.utils.metric_utils import get_metric_names, TrainerMetric
from theflow.utils.metrics_printed_table import print_metrics_table
from theflow.utils.misc_utils import set_random_seed
from theflow.utils.model_utils import contains_nan_or_inf_tensors
from theflow.utils.torch_utils import get_torch_device
from theflow.utils.trainer_utils import (
    append_metrics,
    freeze_layers_regex,
...
class Trainer(BaseTrainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param model: Underlying The Flow model
        :type model: `theflow.models.ecd.ECD`
        ...
        :param skip_save_model: Disables saving model weights and hyperparameters each time the model improves. By
                default The Flow saves model weights after each round of evaluation the validation metric (improves, but
        ...
        :param skip_save_progress: Disables saving progress each round of evaluation. By default The Flow saves weights
        ...
        :param skip_save_log: Disables saving TensorBoard logs. By default The Flow saves logs for the TensorBoard, but if
                it is not needed turning it off can slightly increase the overall speed. (default: False).
        :type skip_save_log: Boolean
        :param callbacks: List of `theflow.callbacks.Callback` objects that provide hooks into the The Flow pipeline.
                (default: None).
        :type callbacks: list
        ...
        :type device: str
        :param config: `theflow.schema.trainer.BaseTrainerConfig` instance that specifies training hyperparameters
                (default: `theflow.schema.trainer.ECDTrainerConfig()`).
        """
    ...
    def train(
        ...
        **kwargs,
        ...
        # https://github.com/theflow-ai/theflow/issues/286
        ...
        try:
            with training_set.initialize_batcher(
                ...
                progress_bar = The FlowProgressBar(self.report_tqdm_to_ray, progress_bar_config, self.is_coordinator())
    ...
    def _train_loop(
        ...
        save_path,
        train_summary_writer,
        progress_bar: The FlowProgressBar,
        training_set,
        validation_set,
        test_set,
        start_time,
