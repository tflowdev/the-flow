...
from theflow.automl.base_config import (
...
from theflow.backend import Backend, initialize_backend
...
from theflow.globals import LUDWIG_VERSION, MODEL_FILE_NAME
...
from theflow.utils.automl.ray_utils import _ray_init
from theflow.utils.automl.utils import _add_transfer_config, get_model_type, set_output_feature_metric
from theflow.utils.data_utils import load_dataset, use_credentials
...
try:
    ...
    raise RuntimeError("ray is not installed. In order to use auto_train please run pip install theflow[ray]") from e
...
class AutoTrainResults:
    ...
    @property
    def best_model(self) -> Optional[The FlowModel]:
        ...
        if ckpt_type == "uri":
            ...
            with use_credentials(self._creds):
                return The FlowModel.load(os.path.join(ckpt_path, MODEL_FILE_NAME))
        else:
...
def auto_train(
    ...
    **kwargs,
    ...
    :param kwargs: additional keyword args passed down to `theflow.hyperopt.run.hyperopt`.
...
def create_auto_config(
    ...
    """Returns an auto-generated The Flow config with the intent of training the best model on given given dataset /
...
def train_with_config(
    ...
    **kwargs,
    ...
    :param config: (dict) optional The Flow configuration to use for training, defaults
...
def cli_init_config(sys_argv):
    ...
    print_theflow("Init Config", LUDWIG_VERSION)
