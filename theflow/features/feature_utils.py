...
from theflow.constants import NAME, PREPROCESSING, SEQUENCE, TEXT, TIMESERIES, TYPE
from theflow.utils.data_utils import hash_dict
from theflow.utils.strings_utils import get_tokenizer_from_registry, UNKNOWN_SYMBOL

SEQUENCE_TYPES = {SEQUENCE, TEXT, TIMESERIES}
FEATURE_NAME_SUFFIX = "__theflow"
...
def get_module_dict_key_from_name(name: str, feature_name_suffix: str = FEATURE_NAME_SUFFIX) -> str:
    ...
    key = name.replace(".", "__theflow_punct_period__")
...
def get_name_from_module_dict_key(key: str, feature_name_suffix_length: int = FEATURE_NAME_SUFFIX_LENGTH) -> str:
    ...
    name = key.replace("__theflow_punct_period__", ".")
    return name[:-feature_name_suffix_length]


class The FlowFeatureDict(torch.nn.Module):
