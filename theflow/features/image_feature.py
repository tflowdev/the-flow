...
from theflow.encoders.image.torchvision import TVModelVariant
from theflow.features.base_feature import BaseFeatureMixin, InputFeature, OutputFeature, PredictModule
from theflow.schema.features.augmentation.base import BaseAugmentationConfig
from theflow.schema.features.augmentation.image import (
...
from theflow.schema.features.image_feature import ImageInputFeatureConfig, ImageOutputFeatureConfig
...
from theflow.utils.augmentation_utils import get_augmentation_op, register_augmentation_op
from theflow.utils.data_utils import get_abs_path
from theflow.utils.dataframe_utils import is_dask_series_or_df
from theflow.utils.fs_utils import has_remote_protocol, upload_h5
from theflow.utils.image_utils import (
...
from theflow.utils.misc_utils import set_default_value
from theflow.utils.types import Series, TorchscriptPreprocessingInput

# constants used for The Flow image preprocessing
...
def _get_torchvision_transform(
    ...
    three channels, which is not always the case with images input into The Flow.
...
def is_torchvision_encoder(encoder_obj: Encoder) -> bool:
    ...
    from theflow.encoders.image.torchvision import TVBaseEncoder
...
class _ImagePreprocessing(torch.nn.Module):
    ...
    def forward(self, v: TorchscriptPreprocessingInput) -> torch.Tensor:
        ...
        if self.torchvision_transform is not None:
            ...
            # perform pre-processing for The Flow defined image encoders
            if torch.jit.isinstance(v, List[torch.Tensor]):
...
class ImageFeatureMixin(BaseFeatureMixin):
    ...
    def _read_image_if_bytes_obj_and_resize(
        ...
        if img.shape[1] != img_height or img.shape[2] != img_width:
            raise ValueError(
                ...
                "https://theflow-ai.github.io/theflow-docs/latest/configuration/features/image_features"
    ...
    def add_feature_data(
        ...
        if torchvision_parameters:
            ...
            # perform The Flow specified transformations
            (
                should_resize,
                width,
                height,
    ...
    def __init__(self, input_feature_config: ImageInputFeatureConfig, encoder_obj=None, **kwargs):
        ...
        if input_feature_config.augmentation:
            ...
            if is_torchvision_encoder(self.encoder_obj):
                ...
                # encoder is a The Flow encoder, determine if standardize_image is set to IMAGENET1K
