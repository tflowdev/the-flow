...
from theflow.constants import AUTO
from theflow.distributed.base import DistributedStrategy
from theflow.modules.optimization_modules import create_optimizer
from theflow.utils.horovod_utils import gather_all_tensors, is_distributed_available
...
class HorovodStrategy(DistributedStrategy):
    ...
    def get_trainer_cls(cls, backend_config: BackendConfig) -> Tuple[Type[DataParallelTrainer], Dict[str, Any]]:
        if not _ray220:
            from theflow.distributed._ray_210_compat import HorovodTrainerRay210
