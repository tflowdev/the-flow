...
from theflow.constants import SPLIT
from theflow.datasets.archives import extract_archive, is_archive, list_archive
from theflow.datasets.dataset_config import DatasetConfig, DatasetFallbackMirror
from theflow.datasets.kaggle import download_kaggle_dataset
from theflow.datasets.utils import model_configs_for_dataset
from theflow.utils.fs_utils import get_default_cache_location, get_fs_and_path
...
@PublicAPI
class DatasetLoader:
    """Base class that defines the default pipeline for loading a theflow dataset.
    ...
    def save_processed(self, dataframe: pd.DataFrame) -> None:
        """Saves transformed dataframe as a flat file theflow can load for training."""
        if not os.path.exists(self.processed_dataset_dir):
            os.makedirs(self.processed_dataset_dir)
