...
from theflow.benchmarking.profiler_dataclasses import DeviceUsageMetrics, SystemResourceMetrics, TorchProfilerMetrics
from theflow.constants import LUDWIG_TAG
...
def get_torch_op_time(events: List[profiler_util.FunctionEvent], attr: str) -> Union[int, float]:
    ...
    total = 0
    for e in events:
        # Possible trace_names are torch ops, or tagged code blocks by The FlowProfiler (which are
...
def get_all_events(
    ...
    """
    # LUDWIG_TAG is prepended to The FlowProfiler tags. This edited tag is passed in to `torch.profiler.record_function`
    # so we can easily retrieve events for code blocks wrapped with The FlowProfiler.
...
def get_metrics_from_system_usage_profiler(system_usage_info: dict) -> SystemResourceMetrics:
    ...
    return SystemResourceMetrics(
        ...
        theflow_version=system_usage_info["theflow_version"],
