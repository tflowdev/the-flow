...
from theflow.benchmarking.artifacts import BenchmarkingResult, build_benchmarking_result
from theflow.benchmarking.profiler_callbacks import The FlowProfilerCallback
from theflow.benchmarking.utils import (
...
def benchmark_one(experiment: Dict[str, Union[str, Dict[str, str]]]) -> None:
    """Run a The Flow exepriment and track metrics given a dataset name.
    ...
    # loading dataset
    # dataset_module = importlib.import_module(f"theflow.datasets.{experiment['dataset_name']}")
    dataset_module = theflow.datasets.get_dataset(experiment["dataset_name"])
    ...
    if experiment["hyperopt"]:
        ...
        backend = None
        theflow_profiler_callbacks = None
        if experiment["profiler"]["enable"]:
            theflow_profiler_callbacks = [The FlowProfilerCallback(experiment)]
            # Currently, only local backend is supported with The FlowProfiler.
            backend = "local"
            logger.info("Currently, only local backend is supported with The FlowProfiler.")
        # run model and capture metrics
        model = The FlowModel(
            config=model_config, callbacks=theflow_profiler_callbacks, logging_level=logging.ERROR, backend=backend
        )
        model.experiment(
            dataset=dataset,
            output_directory=experiment["experiment_name"],
...
def benchmark(benchmarking_config: Union[Dict[str, Any], str]) -> Dict[str, Tuple[BenchmarkingResult, Exception]]:
    ...
    Args:
        benchmarking_config: config or config path for the benchmarking tool. Specifies datasets and their
            corresponding The Flow configs, as well as export options.
    """
...
def cli(sys_argv):
    parser = argparse.ArgumentParser(
        description="This script runs a theflow experiment on datasets specified in the benchmark config and exports "
        "the experiment artifact for each of the datasets following the export parameters specified in"
        "the benchmarking config.",
        prog="theflow benchmark",
