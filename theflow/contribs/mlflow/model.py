...
def get_default_conda_env():
    ...
    # The Flow is not yet available via the default conda channels, so we install it via pip
    return _mlflow_conda_env(
        additional_conda_deps=None,
        additional_pip_deps=[f"theflow=={theflow.__version__}"],
...
def save_model(
    ...
    """Save a The Flow model to a path on the local file system.

    :param theflow_model: The Flow model (an instance of `theflow.api.The FlowModel`_) to be saved.
    ...
    # Save the The Flow model
    theflow_model.save(model_data_path)
    ...
    pyfunc.add_to_model(
        mlflow_model,
        loader_module="theflow.contribs.mlflow.model",
        data=model_data_subpath,
        env=conda_env_subpath,
    )

    schema_keys = {"name", "column", "type"}
    config = theflow_model.config

    mlflow_model.add_flavor(
        FLAVOR_NAME,
        theflow_version=theflow.__version__,
        theflow_schema={
            "input_features": [
...
def log_model(
    ...
    """Log a The Flow model as an MLflow artifact for the current run.
    ...
    Model.log(
        ...
        flavor=theflow.contribs.mlflow.model,
        registered_model_name=registered_model_name,
        conda_env=conda_env,
        signature=signature,
        input_example=input_example,
        await_registration_for=await_registration_for,
        theflow_model=theflow_model,
    )
...
def _load_model(path):
    ...
    return The FlowModel.load(path, backend="local")


def _load_pyfunc(path):
    """Load PyFunc implementation. Called by ``pyfunc.load_pyfunc``.

    :param path: Local filesystem path to the MLflow Model with the ``theflow`` flavor.
    """
    return _The FlowModelWrapper(_load_model(path))


def load_model(model_uri):
    """Load a The Flow model from a local file or a run.
    ...
    :return: A The Flow model (an instance of `theflow.api.The FlowModel`_).
    """
...
class _The FlowModelWrapper:
    def __init__(self, theflow_model):
        self.theflow_model = theflow_model

    def predict(self, dataframe):
        pred_df, _ = self.theflow_model.predict(dataframe)
        return pred_df
...
def log_saved_model(lpath):
    """Log a saved The Flow model as an MLflow artifact.

    :param lpath: Path to saved The Flow model.
    """
    log_model(
        _CopyModel(lpath),
