...
from theflow.constants import SPACE, TRAINING, VALIDATION
...
try:
    ...
    raise RuntimeError(
        ...
        "pip install theflow[viz]"
    ) from e

INT_QUANTILES = 10
FLOAT_QUANTILES = 10

# mapping from RayTune search space to The Flow types (float, int, category) for hyperopt visualizations
...
def hyperopt_report(hyperparameters, hyperopt_results_df, metric, filename_template, float_precision=3):
    ...
    for hp_name, hp_params in hyperparameters.items():
        if hp_params[SPACE] in RAY_TUNE_INT_SPACES:
            ...
            raise ValueError(
                f"{hp_params[SPACE]} search space not supported in The Flow.  "  # noqa: E713
