...
from theflow.models.inference import (
...
from theflow.utils.misc_utils import remove_empty_lines
from theflow.utils.torch_utils import model_size, place_on_device
from theflow.utils.types import TorchAudioTuple, TorchscriptPreprocessingInput
...
class TritonConfigFeature:
    ...
    # The Flow type of the feature, or "tensor"
    theflow_type: str
    ...
    def _get_wrapper_signature_type(self) -> str:
        if self.theflow_type in FEATURES_TO_CAST_AS_STRINGS:
            return "List[str]"
        elif self.theflow_type in [IMAGE, AUDIO, DATE, "tensor"]:
            return {
    ...
    def __post_init__(self):
        ...
        for i, (feature_name, content) in enumerate(self.input_data_example.items()):
            ...
            if self.inference_stage == PREPROCESSOR:
                theflow_type = self.theflow_config[INPUT_FEATURES][i][TYPE]
            self.input_features.append(
                TritonConfigFeature(feature_name, theflow_type, content, self.inference_stage, INPUT, i)
            )
        ...
        for i, (feature_name, content) in enumerate(self.output_data_example.items()):
            ...
            self.output_features.append(
                TritonConfigFeature(feature_name, theflow_type, content, self.inference_stage, OUTPUT, i)
            )
...
def get_inference_modules(model: The FlowModel, predictor_device_type: str) -> List[torch.jit.ScriptModule]:
...
def get_example_input(
    model: The FlowModel, device_types: List[str], data_example: Union[None, pd.DataFrame]
...
def export_triton(
    ...
    model_name: Optional[str] = "theflow_model",
    ...
    # Inputs
    :param model: (The FlowModel) A theflow model.
