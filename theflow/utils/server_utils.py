...
from theflow.utils.data_utils import NumpyEncoder


def serialize_payload(data_source: Union[pd.DataFrame, pd.Series]) -> tuple:
    """
    Generates two dictionaries to be sent via REST API for The Flow prediction
    service.
    ...
    Args:
        data_source: input features to be sent to The Flow server
...
def deserialize_request(form) -> tuple:
    ...
    The Flow predict method and a list of files that will be cleaned up at the end of processing.
