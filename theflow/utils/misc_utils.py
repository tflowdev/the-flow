...
from theflow.constants import PROC_COLUMN
from theflow.globals import DESCRIPTION_FILE_NAME, MODEL_FILE_NAME
from theflow.utils import fs_utils
from theflow.utils.fs_utils import find_non_existing_dir_by_adding_suffix
...
def set_saved_weights_in_checkpoint_flag(config_obj: "ModelConfig"):
    ...
    Next time the model is loaded we will restore pre-trained encoder weights from theflow model (and not load from cache
    or model hub).
    """
...
@DeveloperAPI
def get_commit_hash():
    """If The Flow is run from a git repository, get the commit hash of the current HEAD.

    Returns None if git is not executable in the current environment or The Flow is not run in a git repo.
    """
    try:
