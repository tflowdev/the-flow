"""The Flow string tokenizers including string-based, spacy-based, and huggingface-based implementations.

To add a new tokenizer, 1) implement a subclass of BaseTokenizer and 2) add it to the tokenizer_registry.

Once it's in the registry, tokenizers can be used in a theflow config, e.g..
...
from theflow.constants import PADDING_SYMBOL, UNKNOWN_SYMBOL
from theflow.utils.data_utils import load_json
from theflow.utils.hf_utils import load_pretrained_hf_tokenizer
from theflow.utils.nlp_utils import load_nlp_pipeline, process_text
...
class StringSplitTokenizer(torch.nn.Module):
    ...
    def forward(self, v: Union[str, List[str], torch.Tensor]) -> Any:
        ...
        inputs: List[str] = []
        # The Flow calls map on List[str] objects, so we need to handle individual strings as well.
        if isinstance(v, str):
            inputs.append(v)
        else:
            inputs.extend(v)
...
class _BPETokenizer(torch.nn.Module):
    ...
    def forward(self, v: Union[str, List[str], torch.Tensor]) -> Any:
        ...
        BPE tokenizers from torchtext return ids directly, which is inconsistent with the The Flow tokenizer API. The
...
class BERTTokenizer(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.is_hf_tokenizer:
            # Values used by The Flow extracted from the corresponding HF model.
    ...
    def forward(self, v: Union[str, List[str], torch.Tensor]) -> Any:
        ...
        If the is_hf_tokenizer flag is set to False, then the output follows the The Flow convention, i.e. the output
