...
from theflow.features.feature_registries import get_base_type_registry, get_input_type_registry, get_output_type_registry
...
from theflow.utils.version_transformation import VersionTransformation, VersionTransformationRegistry
...
def register_config_transformation(version: str, prefixes: Union[str, List[str]] = []) -> Callable:
    ...
    Args:
        version: The version to register this transformation with. The earliest theflow version which requires this
                 transformation.
...
def upgrade_config_dict_to_latest_version(config: ModelConfigDict) -> ModelConfigDict:
    """Updates config from an older version of The Flow to the current version. If config does not have a
    "theflow_version" key, all updates are applied.

    Args:
        config: A config saved by an older version of The Flow.

    Returns A new copy of config, upgraded to the current The Flow version. Returns config if config has no
            "theflow_version".
    """
    return config_transformation_registry.update_config(
        config, from_version=config.get("theflow_version", "0.0"), to_version=LUDWIG_VERSION
    )
...
def upgrade_model_progress(model_progress: Dict) -> Dict:
    ...
    # Delete learning-rate related fields removed in https://github.com/theflow-ai/theflow/pull/2877.
    ...
    for parameter in list(config.get(PREPROCESSING, {})):
        if parameter in get_base_type_registry():
            warnings.warn(
                ...
                " to `defaults` section in The Flow config. This will be unsupported in v0.8.",
                DeprecationWarning,
            )
...
def _upgrade_legacy_image_encoders(feature: FeatureConfigDict) -> FeatureConfigDict:
    ...
    # For this version of The Flow, only ECD supported these encoders.
...
def remove_extra_type_param_in_defaults_config(defaults: FeatureTypeDefaultsDict) -> FeatureTypeDefaultsDict:
    ...
    [1]: https://github.com/theflow-ai/theflow/pull/3223
    [2]: https://github.com/theflow-ai/theflow/pull/3258
    """
