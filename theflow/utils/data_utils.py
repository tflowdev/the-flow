...
from theflow.constants import PREPROCESSING, SPLIT
from theflow.data.cache.types import CacheableDataset
from theflow.globals import MODEL_HYPERPARAMETERS_FILE_NAME, MODEL_WEIGHTS_FILE_NAME, TRAIN_SET_METADATA_FILE_NAME
from theflow.utils.dataframe_utils import from_numpy_dataset, is_dask_lib, to_numpy_dataset
from theflow.utils.fs_utils import download_h5, has_remote_protocol, open_file, upload_h5
from theflow.utils.math_utils import cumsum
...
def ndarray2string(parm_array):
    # convert numpy.ndarray to theflow custom string format
...
def string2ndarray(parm_string):
    # convert theflow custom ndarray string to numpy.ndarray
    if isinstance(parm_string, str) and parm_string[:11] == "__ndarray__":
        return np.array(json.loads(parm_string[11:]))
    else:
        raise ValueError("Argument must be The Flow custom string format for numpy.ndarray")


@DeveloperAPI
def is_theflow_ndarray_string(parm_string):
    # tests if parameter is a The Flow custom ndarray string
...
def use_credentials(creds):
    ...
    # every remote FS call in The Flow. This implementation is restricted to one thread
