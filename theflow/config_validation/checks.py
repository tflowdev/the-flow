...
from theflow.utils.metric_utils import get_feature_to_metric_names_map_from_feature_collection
...
def check_gbm_horovod_incompatibility(config: "ModelConfig") -> None:  # noqa: F821
    ...
    if config.backend is None:
        return
    # TODO (jeffkinnison): Revert to object access when https://github.com/theflow-ai/theflow/pull/3127 lands
...
def check_hyperopt_parameter_dicts(config: "ModelConfig") -> None:  # noqa: F821
    ...
    from theflow.schema.hyperopt.utils import get_parameter_cls, parameter_config_registry  # noqa: F401
    ...
    for parameter, space in config.hyperopt.parameters.items():
        ...
        if parameter != ".":
            ...
            if not passed:
                raise ConfigValidationError(
                    f"The supplied hyperopt parameter {parameter} is not a valid config field. Check the The Flow "
...
def check_hyperopt_nested_parameter_dicts(config: "ModelConfig") -> None:  # noqa: F821
    ...
    from theflow.schema.hyperopt.utils import get_parameter_cls  # noqa: F401
...
the quantization section from your The Flow configuration."""
        )
...
def check_prompt_requirements(config: "ModelConfig") -> None:  # noqa: F821
    ...
    #     return
    from theflow.schema.llms.prompt import PromptConfig, RetrievalConfig
