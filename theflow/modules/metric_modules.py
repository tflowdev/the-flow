...
from theflow.constants import (  # RESPONSE,
    ACCURACY,
    ACCURACY_MICRO,
    BINARY,
...
from theflow.distributed import get_current_dist_strategy
from theflow.modules.loss_modules import (
    BWCEWLoss,
    CORNLoss,
    HuberLoss,
...
from theflow.modules.metric_registry import get_metric_objective, get_metric_registry, register_metric
from theflow.schema.features.loss.loss import (
    BWCEWLossConfig,
    CORNLossConfig,
    HuberLossConfig,
...
from theflow.utils.loss_utils import rmspe_loss
from theflow.utils.metric_utils import masked_correct_predictions
from theflow.utils.torch_utils import sequence_length_2D

logger = logging.getLogger(__name__)


class The FlowMetric(Metric, ABC):
    @classmethod
...
class RMSEMetric(MeanSquaredError, The FlowMetric):
...
class PrecisionMetric(BinaryPrecision, The FlowMetric):
    """Precision metric."""
...
class RecallMetric(BinaryRecall, The FlowMetric):
    """Recall metric."""
...
class BinaryAUROCMetric(BinaryAUROC, The FlowMetric):
...
class CategoryAUROCMetric(MulticlassAUROC, The FlowMetric):
    """Area under the receiver operating curve."""
...
class SpecificityMetric(BinarySpecificity, The FlowMetric):
    """Specificity metric."""
...
class MeanMetric(The FlowMetric):
...
class PerplexityMetric(Perplexity, The FlowMetric):
    def __init__(self, **kwargs):
...
# https://github.com/theflow-ai/theflow/issues/3953
class BLEUScoreMetric(BLEUScore, The FlowMetric):
    def __init__(self, **kwargs):
        super().__init__()
...
# https://github.com/theflow-ai/theflow/issues/3953
class ROUGEScoreMetric(ROUGEScore, The FlowMetric):
    def __init__(self, **kwargs):
        super().__init__()
...
# https://github.com/theflow-ai/theflow/issues/3953
class WordErrorRateMetric(WordErrorRate, The FlowMetric):
    def __init__(self, **kwargs):
        super().__init__()
...
# https://github.com/theflow-ai/theflow/issues/3953
class CharErrorRateMetric(CharErrorRate, The FlowMetric):
    def __init__(self, **kwargs):
        super().__init__()


@register_metric(ACCURACY, [BINARY], MAXIMIZE, PREDICTIONS)
class Accuracy(BinaryAccuracy, The FlowMetric):
    """R-squared metric."""
...
class CategoryAccuracy(MulticlassAccuracy, The FlowMetric):
    def __init__(self, num_classes: int, **kwargs):
        super().__init__(num_classes=num_classes)
...
class CategoryAccuracyMicro(MulticlassAccuracy, The FlowMetric):
    def __init__(self, num_classes: int, **kwargs):
...
class HitsAtKMetric(MulticlassAccuracy, The FlowMetric):
...
class MAEMetric(MeanAbsoluteError, The FlowMetric):
    def __init__(self, **kwargs):
        super().__init__()
...
class MSEMetric(MeanSquaredError, The FlowMetric):
    def __init__(self, **kwargs):
        super().__init__()
...
class MAPEMetric(MeanAbsolutePercentageError, The FlowMetric):
    def __init__(self, **kwargs):
        super().__init__()
...
def get_metric_cls(metric_name: str) -> Type[The FlowMetric]:
